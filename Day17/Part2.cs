...
internal static class Part2
{
    ...
    public static void FindHeatLossForSurroundingBlocks(Block currentBlock, HashSet<Block> blocks, int maxX, int maxY)
    {
        ...
        var upY = curY - 1;
        var downY = curY + 1;
        var xFromDestination = maxX - currentBlock.X;
        var yFromDestination = maxY - currentBlock.Y;
        var consecutiveStraightMoves = currentBlock.ConsecutiveStepsInDirection;
        var lastMoveDirection = currentBlock.Direction;
        var directionChangeAllowed = consecutiveStraightMoves >= 4;
        var directionChangeRequired = consecutiveStraightMoves >= 10;
        var allowRight = !(currentBlock.Y == maxY && (lastMoveDirection == Direction.Right ? consecutiveStraightMoves : 0) + xFromDestination < 4);
        var allowDown = !(currentBlock.X == maxX && (lastMoveDirection == Direction.Down ? consecutiveStraightMoves : 0)  + yFromDestination < 4);

        if (!directionChangeAllowed && lastMoveDirection is not Direction.Start)
        {
            var x = lastMoveDirection is Direction.Down or Direction.Up ? curX : lastMoveDirection is Direction.Right ? rightX : leftX;
            var y = lastMoveDirection is Direction.Right or Direction.Left ? curY : lastMoveDirection is Direction.Up ? upY : downY;
            
            if (!(lastMoveDirection == Direction.Right && !allowRight) && !(lastMoveDirection == Direction.Down && !allowDown))
            {
                UpdateBlockWithMinHeatLoss(currentBlock, blocks, lastMoveDirection, x, y);
            }
            return;
        }
        ...
        if (rightX <= maxX && lastMoveDirection != Direction.Left && !(directionChangeRequired && lastMoveDirection == Direction.Right) && allowRight)
        {
        ...
        if (downY <= maxY && lastMoveDirection != Direction.Up && !(directionChangeRequired && lastMoveDirection == Direction.Down) && allowDown)
        {
